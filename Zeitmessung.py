...
def action():
    if Decoder_action.get() == 'active':
        ...
        if finisher > Merker_Finish.get():                            # erst, wenn neue Finisher vorhanden sind, weiter machen
            for k in range(Merker_Finish.get() + 1, finisher +1):     # alle vorhandenen neuen Passingfiles auslesen
                nachricht = str(k)+"\r\n"
...
def start():
    ...
    if Decoder_Status.get() == 'verbunden':                           
        nachricht = "STARTOPERATION\r\n"                              # Switch Decode von Test Mode zu Operation Mode
        s.send(nachricht.encode())
...
def connect():
    ...
    s.connect((IP.ergebnis.get(), Decoder_Port))
    nachricht = "GETSTATUS\r\n"                                       # Decoderstatus abrufen
    s.send(nachricht.encode())
    antwort = s.recv(1024)
    string = antwort.decode()
...
decoder.geometry('800x600')                              # Fenstergröße definieren, eventuell muss man hier zuerst die Bildschirmauflösung abfragenn
